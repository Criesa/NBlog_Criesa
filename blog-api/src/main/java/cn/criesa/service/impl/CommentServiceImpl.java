...
public class CommentServiceImpl implements CommentService {
	...
	@Override
	public List<cn.criesa.entity.Comment> getListByPageAndParentCommentId(Integer page, Long blogId, Long parentCommentId) {
		List<cn.criesa.entity.Comment> comments = commentMapper.getListByPageAndParentCommentId(page, blogId, parentCommentId);
		for (cn.criesa.entity.Comment c : comments) {
			//递归查询子评论及其子评论
			List<cn.criesa.entity.Comment> replyComments = getListByPageAndParentCommentId(page, blogId, c.getId());
	...
	@Override
	public cn.criesa.entity.Comment getCommentById(Long id) {
		cn.criesa.entity.Comment comment = commentMapper.getCommentById(id);
		if (comment == null) {
	...
	public void updateCommentPublishedById(Long commentId, Boolean published) {
		...
		if (!published) {
			List<cn.criesa.entity.Comment> comments = getAllReplyComments(commentId);
			for (cn.criesa.entity.Comment c : comments) {
				hideComment(c);
			}
		}
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public void updateComment(cn.criesa.entity.Comment comment) {
	 ...
	 */
	private void delete(cn.criesa.entity.Comment comment) {
		for (cn.criesa.entity.Comment c : comment.getReplyComments()) {
			delete(c);
		}
	 ...
	 */
	private void hideComment(cn.criesa.entity.Comment comment) {
		for (cn.criesa.entity.Comment c : comment.getReplyComments()) {
			hideComment(c);
		}
	 ...
	 * @return
	 */
	private List<cn.criesa.entity.Comment> getAllReplyComments(Long parentCommentId) {
		List<cn.criesa.entity.Comment> comments = commentMapper.getListByParentCommentId(parentCommentId);
		for (cn.criesa.entity.Comment c : comments) {
			List<cn.criesa.entity.Comment> replyComments = getAllReplyComments(c.getId());
			c.setReplyComments(replyComments);
		}
		return comments;
	}
